...
type PersonData = {
  ...
  notes: string;
  nicknames: string[];
  selectedCrimeCategoryIds: string[];
};
...
export default function EditPersonForm({
  ...
  const [atoll, setAtoll] = useState(person.atoll || "R");
  const [island, setIsland] = useState(person.island);
  ...
  const [selectedCrimeCategories, setSelectedCrimeCategories] = useState<
    string[]
  >(person.selectedCrimeCategoryIds);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Failed to update person record.");
        return;
      }

      setSuccess("Person record updated successfully.");
  ...
  async function deleteRecord() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Failed to delete person record.");
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-sky-950 via-blue-800 to-cyan-600 text-white shadow-xl shadow-blue-900/20">
        <div className="p-6 sm:p-8">
          ...
          <p className="mt-3 max-w-2xl text-sm font-semibold leading-6 text-blue-100">
            Update person information, nicknames, image and crime categories.
          </p>
        </div>
      </div>
      ...
      <SectionCard number="02" title="Nicknames">
      ...
      <SectionCard number="04" title="Crime Categories">
      ...
      <SectionCard number="05" title="Notes">
        <textarea
          value={notes}
