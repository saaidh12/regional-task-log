...
export default function EditYaumiyyaForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`/api/yaumiyya/${record.id}`, {
        method: "PATCH",
        headers: {
      ...
      if (!res.ok) {
        setError(data.error || "Failed to update Yaumiyya.");
        return;
      }

      setSuccess("Yaumiyya updated successfully.");

      setTimeout(() => {
        router.push("/yaumiyya");
        router.refresh();
      }, 700);
    } catch {
  ...
  async function deleteRecord() {
    const ok = window.confirm(
      `Delete "${
        record.meetingTitle || "Yaumiyya record"
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || "Failed to delete Yaumiyya.");
        return;
      }
    ...
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1fr_360px]">
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <SectionCard
          ...
          subtitle="Update date, start time, finished time and title."
        >
        ...
        <div className="sticky bottom-20 z-20 rounded-[1.5rem] border border-slate-200 bg-white/95 p-3 shadow-xl backdrop-blur lg:static lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none">
          <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
            ...
            >
              {loading ? "Saving..." : "Update Yaumiyya"}
            </button>
          </div>
        </div>
      </form>
